...
import math
from splinegeneration import spline_gen
...
class LOSTracking():
    ...
    def execute(self,eta,Wpx,Wpy): 


        Spline_gen = spline_gen()
        self.x_int,self.y_int=Spline_gen.pchip_gen(Wpx,Wpy)
        coeff_list = Spline_gen.execute()
        k =0
        m = 4 # Araç etrafındaki 2 gemi boyundaki çamber
        print('coeff_list:',coeff_list)
    
        dist = np.sqrt((eta[0]-Wpx[k+1])**2+(eta[1]-Wpy[k+1])**2)
        print('dist:',dist)
        if dist<=m:
            coefficient = coeff_list[k][:] 
